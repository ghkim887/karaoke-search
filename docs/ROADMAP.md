...
3. **벤더번호 진짜 충돌 4곡 = 무행동 종결** (tj-6579 / tj-27098 / tj-27416 /
   ...
   무관하게 그대로 둔다.)
4. **3-way 클래스 84 → 85곡 — 유일한 미결 항목 (설계 승인 대기).** Tier E/F의
   dup-J(unique-joysound) 불변식 완화 설계가 필요하다 (리뷰드 쌍으로 한정,
   벤더번호 충돌가드 유지 — #165의 후속 설계 항목). 신규 합류: tj-26145
   "忘れていいの" — B-wave reject였으나 신발견 듀엣 행(joy 1546)으로 뒤집혀,
   위 ky-40449와 같은 joysound 1546에 합류하는 3-way가 된다. **이 PR은 3-way
   어태치 메커니즘(B2 설계)을 구현하지 않는다** — 위 ky-40449 인코딩은 2행
   단독 Tier F 병합일 뿐이며, tj-26145는 별도 설계 승인 후에만 합류한다.

**발효 캐비엇:** 이 PR의 10쌍(forbidden 해제 8 + uncertain→merge 2)과 #166의
46곡을 포함해 어떤 조치든 실제 발효는 다음 재병합 (크롤 재개 또는 v25
재구성) 시점이다. 이 PR에 데이터/코퍼스 재생성은 없다.
