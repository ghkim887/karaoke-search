...
- v2 status: schema simplified to 3 categories (`jpop`/`vocaloid`/`anime`); merger rewritten with two-tier match key + per-field ownership; TJ-direct adapter shipped (catalog JSON API). NamuWiki adapter and frontend wire-up pending. See `docs/superpowers/specs/2026-04-26-karaoke-search-v2-design.md`.
