...
import { DROP_KEY_SET, DROP_LIST, isInDropList } from '../../src/curated/koreanArtistDropList.js';
...
describe('koreanArtistDropList — sidecar JSON staleness check (Fix 2)', () => {
  // The sidecar at `packages/crawler/src/curated/
  // korean-artist-drop-list.json` is a build-time drift-visibility guard
  // (runtime consumers read the built dist, not this JSON).
  ...
  const SIDECAR_PATH = resolve(HERE, '../../src/curated/korean-artist-drop-list.json');
