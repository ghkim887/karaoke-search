...
import { normalizeJoysoundRecord } from '../../../src/adapters/joysound-official/normalizer.js';
import type {
  JoysoundDetail,
  JoysoundListItem,
...
describe('normalizeJoysoundRecord — record shape', () => {
  ...
  it('puts selSongNo into karaoke_numbers.joysound, leaving tj/ky null', () => {
    const rec = normalizeJoysoundRecord({
    ...
    expect(rec.karaoke_numbers).toEqual({ tj: null, ky: null, joysound: '190-001' });
  });
...
describe('normalizeJoysoundRecord — invariants (no Korean fields)', () => {
  ...
  it('emits karaoke_numbers with only the joysound field populated', () => {
    const rec = normalizeJoysoundRecord({
    ...
    expect(rec.karaoke_numbers.joysound).toBe('500-123');
  });
});
