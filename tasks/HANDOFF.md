...
2. **오너 보류(2026-07-10, 사유 포함 — ROADMAP에 기록)**:
   ...
   - 버전 애매 ~7건: 보류 유지.
3. **go만 있으면 즉시 실행 가능(추천 순)**: ①패리티 베이스라인 재생성 정책 구현
   (크롤 워크플로가 재생성+쿼리별 jaccard 델타를 크롤 PR에 표기 — 금요일 크롤 전
   가치 최대) ②TierB 결정적 타이브레이크(현 산출물 보존형 — 순서 의존만 제거,
   "병합 여부" 질문은 브리지 보류와 함께 유보) ③idf drift 문서화 ④delta 힌트
   정책 문서화+가드 ⑤title_ko CSV ~255건 에이전트 사전검토 배치(R1 전례).
4. (시간 게이트) 금요일 주간 크롤: #97 게이트 첫 실전 + 소킹 체크(병합쌍 적용·
   ruby 유입·classifier 골든·167행 퍼지 유지) + 크롤이 blog 페이지를 만지면
   패리티 베이스라인 재생성 필요(3-①이 미구현인 동안 수동). 이후: TJ filter seam
   설계(KOR pro-reject 스텝), classifier Phase 2.
5. (시간 게이트+오너 go) v22 fullCatalog 크롤 — 금요일 검증 후 착수 권고; 승격은
   oci ssh 보류 해제 필요.
