...
class TestSidecarLoad(unittest.TestCase):
    """The override map is loaded from the JSON sidecar
    (`packages/crawler/src/adapters/jpop-playlist-blog/post-category-overrides.json`)
    generated by `scripts/export-post-category-overrides.mjs` at crawler build
    time. Pin the loaded map to the three audited post-ids.
    """

    def test_sidecar_contains_three_audited_post_ids(self) -> None:
        # The retag module's POST_CATEGORY_OVERRIDES is populated at import
        # time from the sidecar JSON. The dict must contain at least the
        # three original audited entries (`101`, `105`, `112`), each mapped
        # to `jpop`. Future audit-justified additions are allowed; silently
        # dropping any of the original three is not.
        self.assertIn('101', retag.POST_CATEGORY_OVERRIDES)
        self.assertIn('105', retag.POST_CATEGORY_OVERRIDES)
        self.assertIn('112', retag.POST_CATEGORY_OVERRIDES)
        self.assertEqual(retag.POST_CATEGORY_OVERRIDES['101'], 'jpop')
        self.assertEqual(retag.POST_CATEGORY_OVERRIDES['105'], 'jpop')
        self.assertEqual(retag.POST_CATEGORY_OVERRIDES['112'], 'jpop')
