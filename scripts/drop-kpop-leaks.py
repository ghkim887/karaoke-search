...
# Reuse the helpers from the anisong ingest so both scripts apply the SAME
# normalization + splitter. Importing via importlib because the filename
...
def main() -> int:
    ...
    for rec in corpus:
        ...
        if _ingest.is_artist_in_drop_list(artist, drop_keys):
            dropped_count += 1
