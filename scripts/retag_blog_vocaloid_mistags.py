...
# Per-post category override map — loaded from the JSON sidecar generated by
# `scripts/export-post-category-overrides.mjs` at crawler build time. The TS
# source-of-truth lives in
# `packages/crawler/src/adapters/jpop-playlist-blog/crawler.ts`
# (`POST_CATEGORY_OVERRIDES`). The sidecar pattern mirrors the Korean-artist
# drop list (`korean-artist-drop-list.json`) — co-located with the TS source,
# tracked in git, and refreshed idempotently by `pnpm --filter @karaoke/crawler
# build`. Keyed on the numeric post-id (the `{artistIdNumber}` segment of
# `blog-{N}-{rowIndex}` ids).
_POST_CATEGORY_OVERRIDES_SIDECAR = (
    REPO_ROOT
    / 'packages'
    / 'crawler'
    / 'src'
    / 'adapters'
    / 'jpop-playlist-blog'
    / 'post-category-overrides.json'
)


def _load_post_category_overrides() -> dict[str, str]:
    """Load the per-post category override map from the JSON sidecar.

    The sidecar is generated by `scripts/export-post-category-overrides.mjs`
    during `pnpm --filter @karaoke/crawler build`. A missing sidecar is a hard
    error — the override map is the script's entire reason for existing, and
    silently degrading to an empty map would let mistagged records ship.
    """
    if not _POST_CATEGORY_OVERRIDES_SIDECAR.exists():
        raise FileNotFoundError(
            f'missing post-category-overrides sidecar at {_POST_CATEGORY_OVERRIDES_SIDECAR} '
            '(run `corepack pnpm --filter @karaoke/crawler build` to generate it)'
        )
    with open(_POST_CATEGORY_OVERRIDES_SIDECAR, encoding='utf-8') as f:
        sidecar = json.load(f)
    overrides = sidecar.get('overrides')
    if not isinstance(overrides, dict):
        raise ValueError(
            f'malformed post-category-overrides sidecar at {_POST_CATEGORY_OVERRIDES_SIDECAR}: '
            f'expected object at `overrides`, got {type(overrides).__name__}'
        )
    return {str(k): str(v) for k, v in overrides.items()}


POST_CATEGORY_OVERRIDES: dict[str, str] = _load_post_category_overrides()
